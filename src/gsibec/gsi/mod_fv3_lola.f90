...
!
  ...
  public :: fv3dxens,fv3dx1ens,fv3dyens,fv3dy1ens,fv3ixens,fv3ixpens,fv3jyens,fv3jypens,a3dxens,a3dx1ens
  public :: a3dyens,a3dy1ens,a3ixens,a3ixpens,a3jyens,a3jypens
...
!$$$ end documentation block  
  ...
  do j=1,nyen
     do i=1,nxen
        u_out(i,j)=half *((u(i,j)*sangvens(i,j)-v(i,j)*sanguens(i,j))/&
                       (canguens(i,j)*sangvens(i,j)-sanguens(i,j)*cangvens(i,j)) &
                       +(u(i,j+1)*sangvens(i+1,j)-v(i+1,j)*sanguens(i,j+1))/&
                       (canguens(i,j+1)*sangvens(i+1,j)-sanguens(i,j+1)*cangvens(i+1,j)))
        v_out(i,j)=half *((u(i,j)*cangvens(i,j)-v(i,j)*canguens(i,j))/&
                       (sanguens(i,j)*cangvens(i,j)-canguens(i,j)*sangvens(i,j)) &
                       +(u(i,j+1)*cangvens(i+1,j)-v(i+1,j)*canguens(i,j+1))/&
                       (sanguens(i,j+1)*cangvens(i+1,j)-canguens(i,j+1)*sangvens(i+1,j)))
     end do
  end do
  return
