...
use m_set_eta, only: set_eta_write
implicit none

integer :: nlevs,mlevs
integer :: k,km,ks,ii,ier
...
if (trim(akbkfname)/="NULL") then
   call set_eta_read(trim(akbkfname),ak8,bk8,ier)
   if (ier/=0) then
      print *, 'Failed attempt to read ak/bk from: ', trim(akbkfname)
      print *, '**** ABORTING ****'
      call exit(1)
   endif
endif
...
subroutine init_
  ...
  if ( argc < 1 ) then
     ...
     print *, " -akbk FILE  - reads ak/bk from file (see note 1)"
     ...
     print *, "   "
     print *, " Notes:  "
     print *, "  1. akbk opt must be exercise with caution in combination  "
...
call set_eta_write ('test_akbk.nc4',ak8,bk8,ier)
